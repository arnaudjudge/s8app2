...
def view_classification_results(train_data, test1, c1, c2, glob_title, title1, title2, extent, test2=None, c3=None, title3=None):
    ...
    ax2.set_xlim([np.min(train_data[:, 0]), np.max(train_data[:, 0])])
    ax2.set_ylim([np.min(train_data[:, 1]), np.max(train_data[:, 1])])
    # ax1.axes.set_aspect('equal')
...
def calcModeleGaussien(data, message=''):
    ...
    matr_cov = np.cov(data.T, rowvar=True)
...
def decorrelate(data, basis):
    ...
    for i in range(dims[0]):
        ...
        decorrelated[i] = np.matmul(data[i], basis)
        m, cov, valpr, vectprop = calcModeleGaussien(decorrelated[i])
    return decorrelated
