...
def hog_factor(image):  # legacy
    fd, hog_image = hog(image, orientations=9, pixels_per_cell=(8, 8),
                        cells_per_block=(2, 2), visualize=True, multichannel=True)
    return np.sum(hog_image)


def top_down_luminosity_ratio(image):   # legacy
    shape = image.shape
...
def left_right_color_difference_ratio(image):   # legacy
    shape = image.shape
    image = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
...
def excess_blue_index(image):   # legacy
